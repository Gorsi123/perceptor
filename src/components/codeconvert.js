...
const Codecon=function(){
    ...
    const [ocode,setOcode]=useState('please paste your code in the input space...')
    ...
    const handleSubmit= async(e)=>{
        ...
        try{
            if(selectedOption==='option0'){alert('please select the language of your code');return;}
            if(selectedOptionfc==='option0'){alert('please select the language in which you want to convert the code');return;}
            const data=await axios.post("http://localhost:5000/api/codecon",{'code':code,'ilang':selectedOption,'olang':selectedOptionfc},config);
    ...
    return <div className="codecon">
        ...
        <div>
            ...
            <textarea name="code-gen" id="cde" cols="30" rows="10" placeholder="paste your code here" 
                value={code}
                onChange={setCode}
                required></textarea>
            ...
            <div>
            <span onClick={handleCopy} className="copy">{copied==false?'copy':'copied' }</span>

            <select id="selectExample" value={selectedOptionfc} onChange={handleSelectChange}>
            <option value="option0">Please select the language</option>
